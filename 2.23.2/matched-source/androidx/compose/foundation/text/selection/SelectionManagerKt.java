...
public final class SelectionManagerKt {
    ...
    public static final long m933calculateSelectionMagnifierCenterAndroidO0kMr_c(@NotNull SelectionManager manager, long j) {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo, boolean z) {
        ...
        Offset offsetM922getCurrentDragPosition_m7T9E = selectionManager.m922getCurrentDragPosition_m7T9E();
        Intrinsics.checkNotNull(offsetM922getCurrentDragPosition_m7T9E);
        float fM1447getXimpl = Offset.m1447getXimpl(layoutCoordinates.mo2812localPositionOfR5De75A(containerLayoutCoordinates, offsetM922getCurrentDragPosition_m7T9E.getPackedValue()));
        long jMo884getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo884getRangeOfLineContainingjx7JFs(offset);
        Rect boundingBox = anchorSelectable$foundation_release.getBoundingBox(TextRange.m3318getMinimpl(jMo884getRangeOfLineContainingjx7JFs));
        Rect boundingBox2 = anchorSelectable$foundation_release.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m3317getMaximpl(jMo884getRangeOfLineContainingjx7JFs) - 1, TextRange.m3318getMinimpl(jMo884getRangeOfLineContainingjx7JFs)));
        float fCoerceIn = RangesKt.coerceIn(fM1447getXimpl, Math.min(boundingBox.getLeft(), boundingBox2.getLeft()), Math.max(boundingBox.getRight(), boundingBox2.getRight()));
        return Math.abs(fM1447getXimpl - fCoerceIn) > ((float) (IntSize.m3935getWidthimpl(j) / 2)) ? Offset.INSTANCE.m1462getUnspecifiedF1C5BW0() : containerLayoutCoordinates.mo2812localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, Offset.m1448getYimpl(anchorSelectable$foundation_release.getBoundingBox(offset).m1477getCenterF1C5BW0())));
    }

    /* JADX INFO: renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m934containsInclusiveUv8p0NA(@NotNull Rect containsInclusive, long j) {
    ...
    public static final AnnotatedString getCurrentSelectedText(@NotNull Selectable selectable, @NotNull Selection selection) {
        ...
        if (selectable.getSelectableId() == selection.getStart().getSelectableId() && selectable.getSelectableId() == selection.getEnd().getSelectableId()) {
            return selection.getHandlesCrossed() ? text.subSequence(selection.getEnd().getOffset(), selection.getStart().getOffset()) : text.subSequence(selection.getStart().getOffset(), selection.getEnd().getOffset());
        }
        if (selectable.getSelectableId() == selection.getStart().getSelectableId()) {
            return selection.getHandlesCrossed() ? text.subSequence(0, selection.getStart().getOffset()) : text.subSequence(selection.getStart().getOffset(), text.length());
        }
        return selection.getHandlesCrossed() ? text.subSequence(selection.getEnd().getOffset(), text.length()) : text.subSequence(0, selection.getEnd().getOffset());
    }
    ...
    public static final Rect visibleBounds(@NotNull LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m1485Rect0a9Yr6o(layoutCoordinates.mo2816windowToLocalMKHz9U(rectBoundsInWindow.m1482getTopLeftF1C5BW0()), layoutCoordinates.mo2816windowToLocalMKHz9U(rectBoundsInWindow.m1476getBottomRightF1C5BW0()));
    }
}
