...
public final class AutoSize {
    ...
    public static void autoConvertDensity(Activity activity, float f, boolean z) {
        ...
        if (designWidth <= ZegoAudioProcessing.ZegoVoiceChangerCategory.NONE) {
            designWidth = f;
        }
        int iRound = Math.round(AutoSizeConfig.getInstance().getInitScaledDensity() * (f + designWidth + (z ? AutoSizeConfig.getInstance().getScreenWidth() : AutoSizeConfig.getInstance().getScreenHeight()))) & LockFreeTaskQueueCore.MAX_CAPACITY_MASK;
        int i = z ? iRound | MODE_ON_WIDTH : iRound & (-1073741825);
        ...
        if (displayMetricsInfo == null) {
            ...
            if (AutoSizeConfig.getInstance().getPrivateFontScale() > ZegoAudioProcessing.ZegoVoiceChangerCategory.NONE) {
    ...
    public static void autoConvertDensityOfCustomAdapt(Activity activity, CustomAdapt customAdapt) {
        ...
        if (sizeInDp <= ZegoAudioProcessing.ZegoVoiceChangerCategory.NONE) {
    ...
    public static void initCompatMultiProcess(Context context) {
        ...
        StringBuilder sbOooO0O0 = OooO00o.OooO0O0("content://");
        sbOooO0O0.append(context.getPackageName());
        sbOooO0O0.append(".autosize-init-provider");
        contentResolver.query(Uri.parse(sbOooO0O0.toString()), null, null, null, null);
    }
