package com.yalla.yalla.common.ui.view;

import android.content.Context;
import android.util.AttributeSet;
import android.widget.ImageView;
import androidx.appcompat.widget.AppCompatImageView;
import androidx.compose.runtime.internal.StabilityInferred;
import com.umeng.analytics.pro.d;
import com.weieyu.yalla.R;
import kotlin.Metadata;
import kotlin.NoWhenBranchMatchedException;
import kotlin.Pair;
import kotlin.jvm.JvmOverloads;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import p254o00ooO0O.o000O0O0;

/* JADX INFO: loaded from: classes2.dex */
@StabilityInferred(parameters = 0)
@Metadata(bv = {}, d1 = {"\u0000,\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u000b\n\u0002\u0010\b\n\u0000\n\u0002\u0010\u0002\n\u0002\b\b\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0006\b\u0007\u0018\u00002\u00020\u0001:\u0001\u0016B'\b\u0007\u0012\u0006\u0010\u0010\u001a\u00020\u000f\u0012\n\b\u0002\u0010\u0012\u001a\u0004\u0018\u00010\u0011\u0012\b\b\u0002\u0010\u0013\u001a\u00020\u0004¢\u0006\u0004\b\u0014\u0010\u0015J\u001a\u0010\u0007\u001a\u00020\u00062\u0012\u0010\u0005\u001a\u000e\u0012\u0004\u0012\u00020\u0003\u0012\u0004\u0012\u00020\u00040\u0002R\"\u0010\u000e\u001a\u00020\u00038\u0006@\u0006X\u0086\u000e¢\u0006\u0012\n\u0004\b\b\u0010\t\u001a\u0004\b\n\u0010\u000b\"\u0004\b\f\u0010\r¨\u0006\u0017"}, d2 = {"Lcom/yalla/yalla/common/ui/view/UserPremiumView;", "Landroidx/appcompat/widget/AppCompatImageView;", "Lkotlin/Pair;", "", "", "premium", "", "setPremium", "Oooo", "Z", "getShowProtection", "()Z", "setShowProtection", "(Z)V", "showProtection", "Landroid/content/Context;", d.R, "Landroid/util/AttributeSet;", "attrs", "defStyle", "<init>", "(Landroid/content/Context;Landroid/util/AttributeSet;I)V", "OooO00o", "app_YallaRelease"}, k = 1, mv = {1, 7, 1})
public final class UserPremiumView extends AppCompatImageView {

    /* JADX INFO: renamed from: OoooO00, reason: collision with root package name */
    @NotNull
    public static final OooO00o f21052OoooO00 = new OooO00o();

    /* JADX INFO: renamed from: Oooo, reason: collision with root package name and from kotlin metadata */
    public boolean showProtection;

    public static final class OooO00o {

        /* JADX INFO: renamed from: com.yalla.yalla.common.ui.view.UserPremiumView$OooO00o$OooO00o, reason: collision with other inner class name */
        public /* synthetic */ class C0224OooO00o {
            public static final /* synthetic */ int[] $EnumSwitchMapping$0;

            static {
                int[] iArr = new int[PremiumLevel.values().length];
                iArr[PremiumLevel.Premium0.ordinal()] = 1;
                iArr[PremiumLevel.Premium1.ordinal()] = 2;
                iArr[PremiumLevel.Premium2.ordinal()] = 3;
                iArr[PremiumLevel.Premium3.ordinal()] = 4;
                iArr[PremiumLevel.Premium4.ordinal()] = 5;
                iArr[PremiumLevel.Premium5.ordinal()] = 6;
                iArr[PremiumLevel.Premium6.ordinal()] = 7;
                iArr[PremiumLevel.Premium7.ordinal()] = 8;
                $EnumSwitchMapping$0 = iArr;
            }
        }

        public final int OooO00o(int i) {
            switch (C0224OooO00o.$EnumSwitchMapping$0[PremiumLevel.INSTANCE.OooO00o(Integer.valueOf(i)).ordinal()]) {
                case 1:
                    return 0;
                case 2:
                    return R.drawable.ic_vip1;
                case 3:
                    return R.drawable.ic_vip2;
                case 4:
                    return R.drawable.ic_vip3;
                case 5:
                    return R.drawable.ic_vip4;
                case 6:
                    return R.drawable.ic_vip5;
                case 7:
                    return R.drawable.ic_vip6;
                case 8:
                    return R.drawable.ic_vip7;
                default:
                    throw new NoWhenBranchMatchedException();
            }
        }

        @NotNull
        public final String OooO0O0(int i) {
            return OooO0OO(PremiumLevel.INSTANCE.OooO00o(Integer.valueOf(i)));
        }

        @NotNull
        public final String OooO0OO(@NotNull PremiumLevel premiumLevel) {
            Intrinsics.checkNotNullParameter(premiumLevel, "premiumLevel");
            switch (C0224OooO00o.$EnumSwitchMapping$0[premiumLevel.ordinal()]) {
                case 1:
                    return "";
                case 2:
                    OooO00o oooO00o = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Patrician);
                case 3:
                    OooO00o oooO00o2 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Knight);
                case 4:
                    OooO00o oooO00o3 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Baron);
                case 5:
                    OooO00o oooO00o4 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Count);
                case 6:
                    OooO00o oooO00o5 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Marquis);
                case 7:
                    OooO00o oooO00o6 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Duke);
                case 8:
                    OooO00o oooO00o7 = UserPremiumView.f21052OoooO00;
                    return o000O0O0.OooO0OO(R.string.Emperor);
                default:
                    throw new NoWhenBranchMatchedException();
            }
        }
    }

    public /* synthetic */ class OooO0O0 {
        public static final /* synthetic */ int[] $EnumSwitchMapping$0;

        static {
            int[] iArr = new int[PremiumLevel.values().length];
            iArr[PremiumLevel.Premium1.ordinal()] = 1;
            iArr[PremiumLevel.Premium2.ordinal()] = 2;
            iArr[PremiumLevel.Premium3.ordinal()] = 3;
            iArr[PremiumLevel.Premium4.ordinal()] = 4;
            iArr[PremiumLevel.Premium5.ordinal()] = 5;
            iArr[PremiumLevel.Premium6.ordinal()] = 6;
            iArr[PremiumLevel.Premium7.ordinal()] = 7;
            iArr[PremiumLevel.Premium0.ordinal()] = 8;
            $EnumSwitchMapping$0 = iArr;
        }
    }

    /* JADX WARN: 'this' call moved to the top of the method (can break code semantics) */
    @JvmOverloads
    public UserPremiumView(@NotNull Context context) {
        this(context, null, 0);
        Intrinsics.checkNotNullParameter(context, "context");
    }

    public final void OooO0OO(int i, int i2) {
        OooO0o0(i == 1, PremiumLevel.INSTANCE.OooO00o(Integer.valueOf(i2)));
    }

    public final void OooO0Oo(boolean z, int i) {
        OooO0o0(z, PremiumLevel.INSTANCE.OooO00o(Integer.valueOf(i)));
    }

    public final void OooO0o0(boolean z, @NotNull PremiumLevel premiumLevel) {
        Intrinsics.checkNotNullParameter(premiumLevel, "premiumLevel");
        if (premiumLevel.getValue() <= 0 || !(z || this.showProtection)) {
            setVisibility(8);
        }
        boolean z2 = false;
        setVisibility(0);
        setScaleType(ImageView.ScaleType.FIT_CENTER);
        int value = premiumLevel.getValue();
        PremiumLevel premiumLevel2 = PremiumLevel.Premium5;
        if (value > premiumLevel2.getValue()) {
            premiumLevel = premiumLevel2;
        }
        if (!z && this.showProtection) {
            z2 = true;
        }
        switch (OooO0O0.$EnumSwitchMapping$0[premiumLevel.ordinal()]) {
            case 1:
                setImageResource(z2 ? R.drawable.ic_vip1_expired : R.drawable.ic_vip1);
                break;
            case 2:
                setImageResource(z2 ? R.drawable.ic_vip2_expired : R.drawable.ic_vip2);
                break;
            case 3:
                setImageResource(z2 ? R.drawable.ic_vip3_expired : R.drawable.ic_vip3);
                break;
            case 4:
                setImageResource(z2 ? R.drawable.ic_vip4_expired : R.drawable.ic_vip4);
                break;
            case 5:
                setImageResource(z2 ? R.drawable.ic_vip5_expired : R.drawable.ic_vip5);
                break;
            case 6:
                setImageResource(z2 ? R.drawable.ic_vip6_expired : R.drawable.ic_vip6);
                break;
            case 7:
                setImageResource(z2 ? R.drawable.ic_vip7_expired : R.drawable.ic_vip7);
                break;
            case 8:
                setVisibility(8);
                break;
        }
    }

    public final boolean getShowProtection() {
        return this.showProtection;
    }

    public final void setPremium(@NotNull Pair<Boolean, Integer> premium) {
        Intrinsics.checkNotNullParameter(premium, "premium");
        OooO0o0(premium.getFirst().booleanValue(), PremiumLevel.INSTANCE.OooO00o(premium.getSecond()));
    }

    public final void setShowProtection(boolean z) {
        this.showProtection = z;
    }

    /* JADX WARN: 'this' call moved to the top of the method (can break code semantics) */
    @JvmOverloads
    public UserPremiumView(@NotNull Context context, @Nullable AttributeSet attributeSet) {
        this(context, attributeSet, 0);
        Intrinsics.checkNotNullParameter(context, "context");
    }

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    @JvmOverloads
    public UserPremiumView(@NotNull Context context, @Nullable AttributeSet attributeSet, int i) {
        super(context, attributeSet, i);
        Intrinsics.checkNotNullParameter(context, "context");
        setScaleType(ImageView.ScaleType.CENTER_INSIDE);
    }
}
