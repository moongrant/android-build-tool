...
public final class AbstractTypeChecker {
    ...
    private final List<SimpleTypeMarker> collectAllSupertypesWithGivenTypeConstructor(TypeCheckerState typeCheckerState, SimpleTypeMarker simpleTypeMarker, TypeConstructorMarker typeConstructorMarker) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbOooO00o = OooO00o.OooO00o("Too many supertypes for type: ", simpleTypeMarker, ". Supertypes = ");
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicySubstitutionSupertypePolicy != null) {
                    ...
                    while (it.hasNext()) {
                        supertypesDeque.add(supertypesPolicySubstitutionSupertypePolicy.mo5506transformType(typeCheckerState, it.next()));
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo5506transformType = supertypesPolicy.mo5506transformType(typeCheckerState, it.next());
                        if (typeSystemContext.isNothingConstructor(typeSystemContext.typeConstructor(simpleTypeMarkerMo5506transformType))) {
                            typeCheckerState.clear();
                            return true;
                        }
    ...
    public final List<SimpleTypeMarker> findCorrespondingSupertypes(@NotNull TypeCheckerState state, @NotNull SimpleTypeMarker subType, @NotNull TypeConstructorMarker superConstructor) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbOooO00o = OooO00o.OooO00o("Too many supertypes for type: ", subType, ". Supertypes = ");
                sbOooO00o.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                throw new IllegalStateException(sbOooO00o.toString().toString());
            }
            SimpleTypeMarker current = supertypesDeque.pop();
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy != null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        supertypesDeque.add(supertypesPolicy.mo5506transformType(state, it.next()));
                    }
                }
            }
        }
