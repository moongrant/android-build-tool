package com.ishumei.l111l11111lIl;

import android.content.Context;
import android.content.SharedPreferences;
import android.text.TextUtils;
import android.util.Log;
import com.google.android.gms.common.internal.ImagesContract;
import com.ishumei.l1111l111111Il.l11l1111I1l;
import com.ishumei.l111l1111llIl.l111l1111lI1l;
import com.ishumei.l111l1111llIl.l111l1111lIl;
import com.ishumei.l111l1111llIl.l11l1111lIIl;
import com.ishumei.smantifraud.SmAntiFraud;
import com.qiniu.android.common.Constants;
import com.umeng.analytics.pro.ak;
import com.yalla.yalla.common.manager.googlepay.GooglePaySignOrMd5;
import java.io.IOException;
import java.util.HashMap;
import java.util.Map;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes2.dex */
public class l1111l111111Il {
    private static final String l1111l111111Il = "sm";
    private static final String l111l11111I1l = "conf";
    private static final String l111l11111Il = "zaq1mko0";
    private static final String l111l11111lIl = "cloudms.conf";
    private volatile l111l11111lIl l111l1111l1Il;
    private String l111l1111lI1l;
    private String l111l1111lIl;
    private Context l111l1111llIl;

    /* JADX INFO: renamed from: com.ishumei.l111l11111lIl.l1111l111111Il$l1111l111111Il, reason: collision with other inner class name */
    public static class C0163l1111l111111Il {
        private static final l1111l111111Il l1111l111111Il = new l1111l111111Il(0);

        private C0163l1111l111111Il() {
        }
    }

    private l1111l111111Il() {
        Context context = com.ishumei.l1111l111111Il.l111l1111llIl.l1111l111111Il.l111l11111Il;
        if (context != null) {
            this.l111l1111llIl = context.getApplicationContext();
        }
    }

    public /* synthetic */ l1111l111111Il(byte b) {
        this();
    }

    public static l1111l111111Il l1111l111111Il() {
        return C0163l1111l111111Il.l1111l111111Il;
    }

    private l111l11111lIl l1111l111111Il(Context context) {
        String strL111l11111lIl = l111l11111lIl(context);
        if (l111l1111lI1l.l111l11111I1l(strL111l11111lIl)) {
            return null;
        }
        JSONObject jSONObject = new JSONObject(strL111l11111lIl);
        l111l11111lIl l111l11111lilL1111l111111Il = l1111l111111Il(jSONObject.getString("data"), jSONObject.getInt("length"), jSONObject.has("enc") ? jSONObject.getInt("enc") : 0, jSONObject.has("ver") ? jSONObject.getInt("ver") : 0);
        l111l11111lilL1111l111111Il.l1111l111111Il(ImagesContract.LOCAL);
        return l111l11111lilL1111l111111Il;
    }

    private l111l11111lIl l1111l111111Il(String str) {
        JSONObject jSONObject;
        try {
            JSONObject jSONObject2 = new JSONObject(str);
            if (1100 != jSONObject2.getInt("code") || (jSONObject = jSONObject2.getJSONObject("detail")) == null || jSONObject.getInt("code") != 0) {
                return null;
            }
            int i = jSONObject.getInt("length");
            int i2 = jSONObject.has("enc") ? jSONObject.getInt("enc") : 0;
            int i3 = jSONObject.has("ver") ? jSONObject.getInt("ver") : 0;
            String string = jSONObject.getString("data");
            l111l11111lIl l111l11111lilL1111l111111Il = l1111l111111Il(string, i, i2, i3);
            l111l11111lilL1111l111111Il.l1111l111111Il("cloud");
            HashMap map = new HashMap();
            map.put("data", string);
            map.put("length", Integer.valueOf(i));
            map.put("enc", Integer.valueOf(i2));
            map.put("ver", Integer.valueOf(i3));
            Context context = this.l111l1111llIl;
            String string2 = l111l1111lIl.l1111l111111Il((Map<?, ?>) map).toString();
            SharedPreferences.Editor editorEdit = context.getSharedPreferences(l111l11111lIl, 0).edit();
            editorEdit.putString(l111l11111I1l, string2);
            if (editorEdit.commit()) {
                return l111l11111lilL1111l111111Il;
            }
            throw new IOException("editor commit failed");
        } catch (Exception unused) {
            return null;
        }
    }

    private static l111l11111lIl l1111l111111Il(String str, int i, int i2, int i3) {
        String strL1111l111111Il;
        if (str == null) {
            return null;
        }
        try {
            byte[] bArrL111l11111Il = l111l1111lIl.l111l11111Il(str);
            if (i2 == 1) {
                byte[] bArrL1111l111111Il = l11l1111lIIl.l1111l111111Il(com.ishumei.l111l1111llIl.l1111l111111Il.l111l11111lIl(l111l11111Il, bArrL111l11111Il, i));
                strL1111l111111Il = new String(bArrL1111l111111Il, 0, bArrL1111l111111Il.length, Constants.UTF_8);
            } else {
                strL1111l111111Il = com.ishumei.l111l1111llIl.l1111l111111Il.l1111l111111Il(l111l11111Il, bArrL111l11111Il, i);
            }
            return i3 == 1 ? l111l11111lIl.l111l11111lIl(strL1111l111111Il) : l111l11111lIl.l111l11111I1l(strL1111l111111Il);
        } catch (Exception unused) {
            return null;
        }
    }

    private static void l1111l111111Il(Context context, String str) throws IOException {
        SharedPreferences.Editor editorEdit = context.getSharedPreferences(l111l11111lIl, 0).edit();
        editorEdit.putString(l111l11111I1l, str);
        if (!editorEdit.commit()) {
            throw new IOException("editor commit failed");
        }
    }

    public static /* synthetic */ void l1111l111111Il(l1111l111111Il l1111l111111il) {
        try {
            if (TextUtils.isEmpty(l1111l111111il.l111l1111lIl)) {
                return;
            }
            com.ishumei.l111l1111l1Il.l1111l111111Il l1111l111111Il2 = com.ishumei.l111l1111l1Il.l1111l111111Il.l1111l111111Il(l1111l111111il.l111l1111lIl);
            l111l11111lIl l111l11111lIl2 = l1111l111111il.l111l11111lIl();
            String strL11l1111I1ll = l111l11111lIl2 == null ? "" : l111l11111lIl2.l11l1111I1ll();
            String str = com.ishumei.l1111l111111Il.l111l1111llIl.l1111l111111Il.l111l1111l1Il;
            JSONObject jSONObject = new JSONObject();
            jSONObject.put("organization", l1111l111111il.l111l1111lI1l);
            JSONObject jSONObject2 = new JSONObject();
            jSONObject2.put(ak.x, "android");
            jSONObject2.put("sdkver", "3.0.6");
            jSONObject2.put(GooglePaySignOrMd5.md5, strL11l1111I1ll);
            jSONObject2.put("enc", 1);
            jSONObject2.put("bb", l11l1111I1l.l1111l111111Il().l111l11111lIl());
            jSONObject2.put("sid", str);
            jSONObject.put("data", jSONObject2);
            l1111l111111il.l111l1111l1Il = l1111l111111il.l1111l111111Il(new com.ishumei.l111l1111l1Il.l111l11111I1l().l1111l111111Il(l1111l111111Il2).l1111l111111Il(jSONObject.toString().getBytes(Constants.UTF_8), null));
        } catch (Exception unused) {
        }
    }

    private l111l11111lIl l111l11111Il() {
        try {
            l111l11111lIl l111l11111lilL1111l111111Il = null;
            String string = this.l111l1111llIl.getSharedPreferences(l111l11111lIl, 0).getString(l111l11111I1l, null);
            if (!l111l1111lI1l.l111l11111I1l(string)) {
                JSONObject jSONObject = new JSONObject(string);
                l111l11111lilL1111l111111Il = l1111l111111Il(jSONObject.getString("data"), jSONObject.getInt("length"), jSONObject.has("enc") ? jSONObject.getInt("enc") : 0, jSONObject.has("ver") ? jSONObject.getInt("ver") : 0);
                l111l11111lilL1111l111111Il.l1111l111111Il(ImagesContract.LOCAL);
            }
            if (l111l11111lilL1111l111111Il != null) {
                return l111l11111lilL1111l111111Il;
            }
        } catch (Exception e) {
            Log.getStackTraceString(e);
        }
        try {
            JSONObject jSONObject2 = new JSONObject("{\"code\":0,\"data\":\"\",\"enc\":1,\"length\":5866,\"ver\":1}");
            l111l11111lIl l111l11111lilL1111l111111Il2 = l1111l111111Il(jSONObject2.getString("data"), jSONObject2.getInt("length"), jSONObject2.has("enc") ? jSONObject2.getInt("enc") : 0, jSONObject2.has("ver") ? jSONObject2.getInt("ver") : 0);
            l111l11111lilL1111l111111Il2.l1111l111111Il("code");
            return l111l11111lilL1111l111111Il2;
        } catch (Exception unused) {
            return new l111l11111lIl();
        }
    }

    private static String l111l11111lIl(Context context) {
        return context.getSharedPreferences(l111l11111lIl, 0).getString(l111l11111I1l, null);
    }

    private l111l11111lIl l111l1111l1Il() {
        JSONObject jSONObject = new JSONObject("{\"code\":0,\"data\":\"\",\"enc\":1,\"length\":5866,\"ver\":1}");
        l111l11111lIl l111l11111lilL1111l111111Il = l1111l111111Il(jSONObject.getString("data"), jSONObject.getInt("length"), jSONObject.has("enc") ? jSONObject.getInt("enc") : 0, jSONObject.has("ver") ? jSONObject.getInt("ver") : 0);
        l111l11111lilL1111l111111Il.l1111l111111Il("code");
        return l111l11111lilL1111l111111Il;
    }

    private void l111l1111llIl() {
        try {
            if (TextUtils.isEmpty(this.l111l1111lIl)) {
                return;
            }
            com.ishumei.l111l1111l1Il.l1111l111111Il l1111l111111Il2 = com.ishumei.l111l1111l1Il.l1111l111111Il.l1111l111111Il(this.l111l1111lIl);
            l111l11111lIl l111l11111lIl2 = l111l11111lIl();
            String strL11l1111I1ll = l111l11111lIl2 == null ? "" : l111l11111lIl2.l11l1111I1ll();
            String str = com.ishumei.l1111l111111Il.l111l1111llIl.l1111l111111Il.l111l1111l1Il;
            JSONObject jSONObject = new JSONObject();
            jSONObject.put("organization", this.l111l1111lI1l);
            JSONObject jSONObject2 = new JSONObject();
            jSONObject2.put(ak.x, "android");
            jSONObject2.put("sdkver", "3.0.6");
            jSONObject2.put(GooglePaySignOrMd5.md5, strL11l1111I1ll);
            jSONObject2.put("enc", 1);
            jSONObject2.put("bb", l11l1111I1l.l1111l111111Il().l111l11111lIl());
            jSONObject2.put("sid", str);
            jSONObject.put("data", jSONObject2);
            this.l111l1111l1Il = l1111l111111Il(new com.ishumei.l111l1111l1Il.l111l11111I1l().l1111l111111Il(l1111l111111Il2).l1111l111111Il(jSONObject.toString().getBytes(Constants.UTF_8), null));
        } catch (Exception unused) {
        }
    }

    public final void l1111l111111Il(SmAntiFraud.SmOption smOption) {
        this.l111l1111lI1l = smOption.l11l1111I1ll();
        this.l111l1111lIl = smOption.l111l1111lIl();
    }

    public final void l111l11111I1l() {
        com.ishumei.l111l11111I1l.l1111l111111Il.l111l11111lIl().l1111l111111Il(new Runnable() { // from class: com.ishumei.l111l11111lIl.l1111l111111Il.1
            @Override // java.lang.Runnable
            public final void run() {
                l1111l111111Il.l1111l111111Il(l1111l111111Il.this);
            }
        }, 7);
    }

    public final l111l11111lIl l111l11111lIl() {
        if (this.l111l1111l1Il == null) {
            synchronized (l1111l111111Il.class) {
                if (this.l111l1111l1Il == null) {
                    this.l111l1111l1Il = l111l11111Il();
                }
            }
        }
        return this.l111l1111l1Il;
    }
}
