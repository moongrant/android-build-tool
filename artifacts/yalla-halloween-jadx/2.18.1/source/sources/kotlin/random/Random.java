...
public abstract class Random {
    ...
    public byte[] nextBytes(@NotNull byte[] array, int fromIndex, int toIndex) {
        ...
        if (!(new IntRange(0, array.length).contains(fromIndex) && new IntRange(0, array.length).contains(toIndex))) {
            throw new IllegalArgumentException(o0O0ooO.OooO00o(o000O0Oo.OooO0O0("fromIndex (", fromIndex, ") or toIndex (", toIndex, ") are out of range: 0.."), array.length, '.').toString());
        }
        if (!(fromIndex <= toIndex)) {
            throw new IllegalArgumentException(Oooo0.OooO00o("fromIndex (", fromIndex, ") must be not greater than toIndex (", toIndex, ").").toString());
        }
        ...
        if (Double.isInfinite(d)) {
            if ((Double.isInfinite(from) || Double.isNaN(from)) ? false : true) {
                if ((Double.isInfinite(until) || Double.isNaN(until)) ? false : true) {
                    double d2 = 2;
                    double dNextDouble2 = ((until / d2) - (from / d2)) * nextDouble();
