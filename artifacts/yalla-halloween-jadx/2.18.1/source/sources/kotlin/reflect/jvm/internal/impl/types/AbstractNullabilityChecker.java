...
public final class AbstractNullabilityChecker {
        ...
        if (!((typeSystemContext.isClassType(type) && !typeSystemContext.isMarkedNullable(type)) || typeSystemContext.isDefinitelyNotNullType(type))) {
            ...
            while (!supertypesDeque.isEmpty()) {
                if (supertypesSet.size() > 1000) {
                    StringBuilder sbOooO0O0 = OooO00o.OooO0O0("Too many supertypes for type: ", type, ". Supertypes = ");
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (supertypesPolicy2 == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker simpleTypeMarkerMo1798transformType = supertypesPolicy2.mo1798transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(simpleTypeMarkerMo1798transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo1798transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo1798transformType)) {
    ...
    public final boolean hasPathByNotMarkedNullableNodes(@NotNull TypeCheckerState state, @NotNull SimpleTypeMarker start, @NotNull TypeConstructorMarker end) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbOooO0O0 = OooO00o.OooO0O0("Too many supertypes for type: ", start, ". Supertypes = ");
                sbOooO0O0.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                throw new IllegalStateException(sbOooO0O0.toString().toString());
            }
            SimpleTypeMarker current = supertypesDeque.pop();
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1798transformType = supertypesPolicy.mo1798transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo1798transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo1798transformType);
