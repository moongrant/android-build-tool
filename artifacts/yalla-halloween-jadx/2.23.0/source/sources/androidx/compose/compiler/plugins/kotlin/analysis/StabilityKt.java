...
public final class StabilityKt {
    ...
    public static final Stability stabilityOf(@NotNull IrExpression expr) {
        ...
        Stability stabilityStabilityOf2 = stabilityOf(expr.getType());
        ...
        if (expr instanceof IrCall) {
            return stabilityOf((IrCall) expr, stabilityStabilityOf2);
        }
        ...
        List<IrExpression> statements = ((IrComposite) expr).getStatements();
        boolean z = true;
        if (!(statements instanceof Collection) || !statements.isEmpty()) {
            for (IrExpression irExpression : statements) {
