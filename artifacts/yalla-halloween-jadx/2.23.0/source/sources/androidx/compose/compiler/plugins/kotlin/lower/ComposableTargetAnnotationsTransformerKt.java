...
public final class ComposableTargetAnnotationsTransformerKt {
    ...
    @NotNull
    public static final InferenceNode inferenceNodeOf(@NotNull IrElement element, @NotNull ComposableTargetAnnotationsTransformer transformer) {
        Intrinsics.checkNotNullParameter(element, "element");
        Intrinsics.checkNotNullParameter(transformer, "transformer");
        if (element instanceof IrFunction) {
            return new InferenceFunctionDeclarationNode(transformer, (IrFunction) element);
        }
        if (element instanceof IrFunctionExpression) {
            return new InferenceFunctionExpressionNode(transformer, (IrFunctionExpression) element);
        }
        if (element instanceof IrTypeOperatorCall) {
            return inferenceNodeOf(((IrTypeOperatorCall) element).getArgument(), transformer);
        }
        if (element instanceof IrCall) {
            return new InferenceCallExpression(transformer, (IrCall) element);
        }
        return element instanceof IrExpression ? new InferenceElementExpression(transformer, (IrExpression) element) : new InferenceUnknownElement(element);
    }
