...
public final class InferenceCallExpression extends InferenceNode {
    ...
    public InferenceCallExpression(@NotNull ComposableTargetAnnotationsTransformer transformer, @NotNull IrCall element) {
        super(null);
        Intrinsics.checkNotNullParameter(transformer, "transformer");
        Intrinsics.checkNotNullParameter(element, "element");
        this.transformer = transformer;
        this.element = element;
        boolean zIsComposableSingletonGetter = transformer.isComposableSingletonGetter(getElement());
        this.isSingletonLambda = zIsComposableSingletonGetter;
        boolean zHasTransformedLambda$compiler_hosted = transformer.hasTransformedLambda$compiler_hosted(getElement());
        this.isTransformedLambda = zHasTransformedLambda$compiler_hosted;
        this.function = zIsComposableSingletonGetter ? transformer.inferenceFunctionOf((IrFunction) transformer.singletonFunctionExpression$compiler_hosted(getElement()).getFunction()) : zHasTransformedLambda$compiler_hosted ? transformer.inferenceFunctionOf((IrFunction) transformer.transformedLambda$compiler_hosted((IrElement) getElement()).getFunction()) : transformer.inferenceFunctionTypeOf(getElement().getType());
    }
