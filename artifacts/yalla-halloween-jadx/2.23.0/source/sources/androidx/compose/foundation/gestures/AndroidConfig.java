...
final class AndroidConfig implements ScrollConfig {
    ...
    public long mo254calculateMouseWheelScroll8xgXZGE(@NotNull Density calculateMouseWheelScroll, @NotNull PointerEvent event, long j) {
        Intrinsics.checkNotNullParameter(calculateMouseWheelScroll, "$this$calculateMouseWheelScroll");
        Intrinsics.checkNotNullParameter(event, "event");
        List<PointerInputChange> changes = event.getChanges();
        Offset offsetM1425boximpl = Offset.m1425boximpl(Offset.INSTANCE.m1452getZeroF1C5BW0());
        int size = changes.size();
        for (int i = 0; i < size; i++) {
            offsetM1425boximpl = Offset.m1425boximpl(Offset.m1441plusMKHz9U(offsetM1425boximpl.getPackedValue(), changes.get(i).getScrollDelta()));
        }
        return Offset.m1443timestuRUvjQ(offsetM1425boximpl.getPackedValue(), -calculateMouseWheelScroll.mo324toPx0680j_4(Dp.m3765constructorimpl(64)));
    }
}
