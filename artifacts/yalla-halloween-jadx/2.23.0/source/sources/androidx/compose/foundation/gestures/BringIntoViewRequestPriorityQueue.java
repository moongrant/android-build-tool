...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (first <= last) {
            while (true) {
                this.requests.getContent()[first].getContinuation().resumeWith(Result.m4213constructorimpl(Unit.INSTANCE));
                if (first == last) {
                    break;
                } else {
                    first++;
    ...
    public final void resumeAndRemoveWhile(@NotNull Function1<? super Rect, Boolean> block) {
        ...
        while (this.requests.isNotEmpty() && block.invoke(((ContentInViewModifier.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            ((ContentInViewModifier.Request) this.requests.removeAt(this.requests.getSize() - 1)).getContinuation().resumeWith(Result.m4213constructorimpl(Unit.INSTANCE));
        }
    }
}
