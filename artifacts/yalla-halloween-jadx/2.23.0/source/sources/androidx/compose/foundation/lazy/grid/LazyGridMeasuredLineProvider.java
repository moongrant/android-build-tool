...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m614childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        return this.isVertical ? Constraints.INSTANCE.m3743fixedWidthOenEA2s(iCoerceAtLeast) : Constraints.INSTANCE.m3742fixedHeightOenEA2s(iCoerceAtLeast);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            int iM593getCurrentLineSpanimpl = GridItemSpan.m593getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i3).getPackedValue());
            LazyGridMeasuredItem lazyGridMeasuredItemM613getAndMeasure3p2s80s = this.measuredItemProvider.m613getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i3, i, m614childConstraintsJhjzzOo$foundation_release(i2, iM593getCurrentLineSpanimpl));
            i2 += iM593getCurrentLineSpanimpl;
            Unit unit = Unit.INSTANCE;
            lazyGridMeasuredItemArr[i3] = lazyGridMeasuredItemM613getAndMeasure3p2s80s;
        }
    ...
    public final long m615itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m614childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
}
