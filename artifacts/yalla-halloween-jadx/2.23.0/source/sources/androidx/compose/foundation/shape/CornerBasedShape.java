...
public abstract class CornerBasedShape implements Shape {
    ...
    public abstract Outline mo710createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, @NotNull LayoutDirection layoutDirection);

    @Override // androidx.compose.ui.graphics.Shape
    @NotNull
    /* JADX INFO: renamed from: createOutline-Pq9zytI */
    public final Outline mo218createOutlinePq9zytI(long size, @NotNull LayoutDirection layoutDirection, @NotNull Density density) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        Intrinsics.checkNotNullParameter(density, "density");
        float fMo718toPxTmRCtEA = this.topStart.mo718toPxTmRCtEA(size, density);
        float fMo718toPxTmRCtEA2 = this.topEnd.mo718toPxTmRCtEA(size, density);
        float fMo718toPxTmRCtEA3 = this.bottomEnd.mo718toPxTmRCtEA(size, density);
        float fMo718toPxTmRCtEA4 = this.bottomStart.mo718toPxTmRCtEA(size, density);
        float fM1504getMinDimensionimpl = Size.m1504getMinDimensionimpl(size);
        float f = fMo718toPxTmRCtEA + fMo718toPxTmRCtEA4;
        if (f > fM1504getMinDimensionimpl) {
            float f2 = fM1504getMinDimensionimpl / f;
            fMo718toPxTmRCtEA *= f2;
            fMo718toPxTmRCtEA4 *= f2;
        }
        float f3 = fMo718toPxTmRCtEA4;
        float f4 = fMo718toPxTmRCtEA2 + fMo718toPxTmRCtEA3;
        if (f4 > fM1504getMinDimensionimpl) {
            float f5 = fM1504getMinDimensionimpl / f4;
            fMo718toPxTmRCtEA2 *= f5;
            fMo718toPxTmRCtEA3 *= f5;
        }
        if (fMo718toPxTmRCtEA >= 0.0f && fMo718toPxTmRCtEA2 >= 0.0f && fMo718toPxTmRCtEA3 >= 0.0f && f3 >= 0.0f) {
            return mo710createOutlineLjSzlW0(size, fMo718toPxTmRCtEA, fMo718toPxTmRCtEA2, fMo718toPxTmRCtEA3, f3, layoutDirection);
        }
        throw new IllegalArgumentException(("Corner size in Px can't be negative(topStart = " + fMo718toPxTmRCtEA + ", topEnd = " + fMo718toPxTmRCtEA2 + ", bottomEnd = " + fMo718toPxTmRCtEA3 + ", bottomStart = " + f3 + ")!").toString());
    }
