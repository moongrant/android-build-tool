...
public final class TextLayoutResultProxyKt {
    ...
    public static final long m832coerceIn3MmeM6k(long j, Rect rect) {
        float right;
        float bottom;
        if (Offset.m1436getXimpl(j) < rect.getLeft()) {
            right = rect.getLeft();
        } else {
            right = Offset.m1436getXimpl(j) > rect.getRight() ? rect.getRight() : Offset.m1436getXimpl(j);
        }
        if (Offset.m1437getYimpl(j) < rect.getTop()) {
            bottom = rect.getTop();
        } else {
            bottom = Offset.m1437getYimpl(j) > rect.getBottom() ? rect.getBottom() : Offset.m1437getYimpl(j);
        }
