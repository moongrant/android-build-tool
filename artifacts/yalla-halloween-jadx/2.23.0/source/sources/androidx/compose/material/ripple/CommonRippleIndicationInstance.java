...
public final class CommonRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private final void m1270drawRipples4WTKRHQ(DrawScope drawScope, long j) {
        Iterator<Map.Entry<PressInteraction.Press, RippleAnimation>> it = this.ripples.entrySet().iterator();
        while (it.hasNext()) {
            RippleAnimation value = it.next().getValue();
            float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
            if (!(pressedAlpha == 0.0f)) {
                value.m1271draw4WTKRHQ(drawScope, Color.m1669copywmQWz5c$default(j, pressedAlpha, 0.0f, 0.0f, 0.0f, 14, null));
            }
        }
    }
    ...
    public void addRipple(@NotNull PressInteraction.Press interaction, @NotNull CoroutineScope scope) {
        ...
        RippleAnimation rippleAnimation = new RippleAnimation(this.bounded ? Offset.m1425boximpl(interaction.getPressPosition()) : null, this.radius, this.bounded, null);
    ...
    public void drawIndication(@NotNull ContentDrawScope contentDrawScope) {
        ...
        m1270drawRipples4WTKRHQ(contentDrawScope, jM1680unboximpl);
    }
