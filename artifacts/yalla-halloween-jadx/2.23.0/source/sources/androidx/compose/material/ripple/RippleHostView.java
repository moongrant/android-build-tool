...
public final class RippleHostView extends View {
    ...
    private final void setRippleState(boolean pressed) {
        ...
        if (pressed || jLongValue >= MinimumRippleStateChangeTime) {
            ...
            this.resetRippleRunnable = oooO;
            postDelayed(oooO, ResetRippleDelayDuration);
        }
    ...
    public final void m1274addRippleKOepWvA(@NotNull PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, @NotNull Function0<Unit> onInvalidateRipple) {
        ...
        m1275updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m1436getXimpl(interaction.getPressPosition()), Offset.m1437getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1275updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.trySetRadius(radius);
        unprojectedRipple.m1282setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt.roundToInt(Size.m1505getWidthimpl(size)), MathKt.roundToInt(Size.m1502getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
