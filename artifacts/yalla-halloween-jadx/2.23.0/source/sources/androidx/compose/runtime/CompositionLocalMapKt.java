...
public final class CompositionLocalMapKt {
    ...
    public static final PersistentCompositionLocalMap compositionLocalMapOf(@NotNull ProvidedValue<?>[] values, @NotNull PersistentCompositionLocalMap parentScope, @Nullable Composer composer, int i) {
        ...
        for (ProvidedValue<?> providedValue : values) {
            ...
            if (providedValue.getCanOverride() || !contains(parentScope, providedValue.getCompositionLocal())) {
                ...
                builderBuilder2.put(compositionLocal, providedValue.getCompositionLocal().provided$runtime_release(providedValue.getValue(), composer, 8));
            }
    ...
    public static final <T> T read(@NotNull PersistentCompositionLocalMap persistentCompositionLocalMap, @NotNull CompositionLocal<T> key) {
        Intrinsics.checkNotNullParameter(persistentCompositionLocalMap, "<this>");
        Intrinsics.checkNotNullParameter(key, "key");
        return contains(persistentCompositionLocalMap, key) ? (T) getValueOf(persistentCompositionLocalMap, key) : key.getDefaultValueHolder$runtime_release().getValue();
    }
}
