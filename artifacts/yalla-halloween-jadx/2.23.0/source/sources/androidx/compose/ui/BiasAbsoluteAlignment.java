...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo1326alignKFBX0sM(long size, long space, @NotNull LayoutDirection layoutDirection) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        long jIntSize = IntSizeKt.IntSize(IntSize.m3925getWidthimpl(space) - IntSize.m3925getWidthimpl(size), IntSize.m3924getHeightimpl(space) - IntSize.m3924getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(MathKt.roundToInt((this.horizontalBias + f) * (IntSize.m3925getWidthimpl(jIntSize) / 2.0f)), MathKt.roundToInt((f + this.verticalBias) * (IntSize.m3924getHeightimpl(jIntSize) / 2.0f)));
    }
