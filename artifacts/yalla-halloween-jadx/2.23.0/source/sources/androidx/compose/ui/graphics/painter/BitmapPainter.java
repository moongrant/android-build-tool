...
public final class BitmapPainter extends Painter {
    ...
    private final long m2199validateSizeN5eqBDc(long srcOffset, long srcSize) {
        if (IntOffset.m3883getXimpl(srcOffset) >= 0 && IntOffset.m3884getYimpl(srcOffset) >= 0 && IntSize.m3925getWidthimpl(srcSize) >= 0 && IntSize.m3924getHeightimpl(srcSize) >= 0 && IntSize.m3925getWidthimpl(srcSize) <= this.image.getWidth() && IntSize.m3924getHeightimpl(srcSize) <= this.image.getHeight()) {
            return srcSize;
        }
    ...
    public boolean equals(@Nullable Object other) {
        ...
        return Intrinsics.areEqual(this.image, bitmapPainter.image) && IntOffset.m3882equalsimpl0(this.srcOffset, bitmapPainter.srcOffset) && IntSize.m3923equalsimpl0(this.srcSize, bitmapPainter.srcSize) && FilterQuality.m1761equalsimpl0(this.filterQuality, bitmapPainter.filterQuality);
    }
    ...
    public long mo2201getIntrinsicSizeNHjbRc() {
        return IntSizeKt.m3935toSizeozmzZPI(this.size);
    }

    public int hashCode() {
        return FilterQuality.m1762hashCodeimpl(this.filterQuality) + ((IntSize.m3926hashCodeimpl(this.srcSize) + ((IntOffset.m3885hashCodeimpl(this.srcOffset) + (this.image.hashCode() * 31)) * 31)) * 31);
    }

    @Override // androidx.compose.ui.graphics.painter.Painter
    public void onDraw(@NotNull DrawScope drawScope) {
        Intrinsics.checkNotNullParameter(drawScope, "<this>");
        OooO0O0.OooOoO(drawScope, this.image, this.srcOffset, this.srcSize, 0L, IntSizeKt.IntSize(MathKt.roundToInt(Size.m1505getWidthimpl(drawScope.mo2108getSizeNHjbRc())), MathKt.roundToInt(Size.m1502getHeightimpl(drawScope.mo2108getSizeNHjbRc()))), this.alpha, null, this.colorFilter, 0, this.filterQuality, 328, null);
    }

    /* JADX INFO: renamed from: setFilterQuality-vDHp3xo$ui_graphics_release, reason: not valid java name */
    public final void m2202setFilterQualityvDHp3xo$ui_graphics_release(int i) {
        this.filterQuality = i;
    }

    @NotNull
    public String toString() {
        return "BitmapPainter(image=" + this.image + ", srcOffset=" + ((Object) IntOffset.m3890toStringimpl(this.srcOffset)) + ", srcSize=" + ((Object) IntSize.m3928toStringimpl(this.srcSize)) + ", filterQuality=" + ((Object) FilterQuality.m1763toStringimpl(this.filterQuality)) + ')';
    }
    ...
    public /* synthetic */ BitmapPainter(ImageBitmap imageBitmap, long j, long j2, int i, DefaultConstructorMarker defaultConstructorMarker) {
        this(imageBitmap, (i & 2) != 0 ? IntOffset.INSTANCE.m3893getZeronOccac() : j, (i & 4) != 0 ? IntSizeKt.IntSize(imageBitmap.getWidth(), imageBitmap.getHeight()) : j2, null);
    }
    ...
    private BitmapPainter(ImageBitmap image, long j, long j2) {
        ...
        this.srcOffset = j;
        this.srcSize = j2;
        this.filterQuality = FilterQuality.INSTANCE.m1766getLowfv9h1I();
        this.size = m2199validateSizeN5eqBDc(j, j2);
        this.alpha = 1.0f;
    }
}
