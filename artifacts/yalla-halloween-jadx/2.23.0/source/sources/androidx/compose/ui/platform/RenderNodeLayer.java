...
public final class RenderNodeLayer implements OwnedLayer, GraphicLayerInfo {
    ...
    public RenderNodeLayer(@NotNull AndroidComposeView ownerView, @NotNull Function1<? super Canvas, Unit> drawBlock, @NotNull Function0<Unit> invalidateParentLayer) {
        ...
        this.transformOrigin = TransformOrigin.INSTANCE.m2033getCenterSzJe1aQ();
    ...
    public void drawLayer(@NotNull Canvas canvas) {
        ...
        canvas.mo1533concat58bKbWc(this.matrixCache.m3109calculateMatrixGrdbGEg(this.renderNode));
    ...
    public void mo3039inverseTransform58bKbWc(@NotNull float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        float[] fArrM3108calculateInverseMatrixbWbORWo = this.matrixCache.m3108calculateInverseMatrixbWbORWo(this.renderNode);
        if (fArrM3108calculateInverseMatrixbWbORWo != null) {
            androidx.compose.ui.graphics.Matrix.m1893timesAssign58bKbWc(matrix, fArrM3108calculateInverseMatrixbWbORWo);
        }
    }

    @Override // androidx.compose.ui.node.OwnedLayer
    /* JADX INFO: renamed from: isInLayer-k-4lQ0M */
    public boolean mo3040isInLayerk4lQ0M(long position) {
        float fM1436getXimpl = Offset.m1436getXimpl(position);
        float fM1437getYimpl = Offset.m1437getYimpl(position);
        if (this.renderNode.getClipToBounds()) {
            return 0.0f <= fM1436getXimpl && fM1436getXimpl < ((float) this.renderNode.getWidth()) && 0.0f <= fM1437getYimpl && fM1437getYimpl < ((float) this.renderNode.getHeight());
        }
        if (this.renderNode.getClipToOutline()) {
            return this.outlineResolver.m3131isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mapBounds(@NotNull MutableRect rect, boolean inverse) {
        ...
        if (!inverse) {
            androidx.compose.ui.graphics.Matrix.m1884mapimpl(this.matrixCache.m3109calculateMatrixGrdbGEg(this.renderNode), rect);
            return;
        }
        float[] fArrM3108calculateInverseMatrixbWbORWo = this.matrixCache.m3108calculateInverseMatrixbWbORWo(this.renderNode);
        if (fArrM3108calculateInverseMatrixbWbORWo == null) {
            ...
            androidx.compose.ui.graphics.Matrix.m1884mapimpl(fArrM3108calculateInverseMatrixbWbORWo, rect);
        }
    }

    @Override // androidx.compose.ui.node.OwnedLayer
    /* JADX INFO: renamed from: mapOffset-8S9VItk */
    public long mo3041mapOffset8S9VItk(long point, boolean inverse) {
        if (!inverse) {
            return androidx.compose.ui.graphics.Matrix.m1882mapMKHz9U(this.matrixCache.m3109calculateMatrixGrdbGEg(this.renderNode), point);
        }
        float[] fArrM3108calculateInverseMatrixbWbORWo = this.matrixCache.m3108calculateInverseMatrixbWbORWo(this.renderNode);
        return fArrM3108calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m1882mapMKHz9U(fArrM3108calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m1450getInfiniteF1C5BW0();
    }

    @Override // androidx.compose.ui.node.OwnedLayer
    /* JADX INFO: renamed from: move--gyyYBs */
    public void mo3042movegyyYBs(long position) {
        int left = this.renderNode.getLeft();
        int top = this.renderNode.getTop();
        int iM3883getXimpl = IntOffset.m3883getXimpl(position);
        int iM3884getYimpl = IntOffset.m3884getYimpl(position);
        if (left == iM3883getXimpl && top == iM3884getYimpl) {
            return;
        }
        if (left != iM3883getXimpl) {
            this.renderNode.offsetLeftAndRight(iM3883getXimpl - left);
        }
        if (top != iM3884getYimpl) {
            this.renderNode.offsetTopAndBottom(iM3884getYimpl - top);
        }
        triggerRepaint();
    ...
    public void mo3043resizeozmzZPI(long size) {
        int iM3925getWidthimpl = IntSize.m3925getWidthimpl(size);
        int iM3924getHeightimpl = IntSize.m3924getHeightimpl(size);
        float f = iM3925getWidthimpl;
        this.renderNode.setPivotX(TransformOrigin.m2028getPivotFractionXimpl(this.transformOrigin) * f);
        float f2 = iM3924getHeightimpl;
        this.renderNode.setPivotY(TransformOrigin.m2029getPivotFractionYimpl(this.transformOrigin) * f2);
        DeviceRenderNode deviceRenderNode = this.renderNode;
        if (deviceRenderNode.setPosition(deviceRenderNode.getLeft(), this.renderNode.getTop(), this.renderNode.getLeft() + iM3925getWidthimpl, this.renderNode.getTop() + iM3924getHeightimpl)) {
            this.outlineResolver.m3132updateuvyYCjk(SizeKt.Size(f, f2));
    ...
    public void mo3044transform58bKbWc(@NotNull float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        androidx.compose.ui.graphics.Matrix.m1893timesAssign58bKbWc(matrix, this.matrixCache.m3109calculateMatrixGrdbGEg(this.renderNode));
    }
    ...
    public void mo3045updateLayerPropertiesdDxrwY(float scaleX, float scaleY, float alpha, float translationX, float translationY, float shadowElevation, float rotationX, float rotationY, float rotationZ, float cameraDistance, long transformOrigin, @NotNull Shape shape, boolean clip, @Nullable RenderEffect renderEffect, long ambientShadowColor, long spotShadowColor, int compositingStrategy, @NotNull LayoutDirection layoutDirection, @NotNull Density density) {
        ...
        this.renderNode.setAmbientShadowColor(ColorKt.m1724toArgb8_81llA(ambientShadowColor));
        this.renderNode.setSpotShadowColor(ColorKt.m1724toArgb8_81llA(spotShadowColor));
        ...
        this.renderNode.setPivotX(TransformOrigin.m2028getPivotFractionXimpl(transformOrigin) * this.renderNode.getWidth());
        this.renderNode.setPivotY(TransformOrigin.m2029getPivotFractionYimpl(transformOrigin) * this.renderNode.getHeight());
        ...
        this.renderNode.mo3094setCompositingStrategyaDBOjCE(compositingStrategy);
