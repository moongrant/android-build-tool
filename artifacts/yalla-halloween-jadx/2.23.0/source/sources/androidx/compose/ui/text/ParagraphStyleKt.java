...
public final class ParagraphStyleKt {
    ...
    public static final ParagraphStyle m3228fastMergeHtYhynw(@NotNull ParagraphStyle fastMerge, @Nullable TextAlign textAlign, @Nullable TextDirection textDirection, long j, @Nullable TextIndent textIndent, @Nullable PlatformParagraphStyle platformParagraphStyle, @Nullable LineHeightStyle lineHeightStyle, @Nullable LineBreak lineBreak, @Nullable Hyphens hyphens, @Nullable TextMotion textMotion) {
        ...
        if (textAlign2 == null || Intrinsics.areEqual(textAlign2, fastMerge.getTextAlign())) {
            if (!TextUnitKt.m3964isUnspecifiedR2X_6o(j)) {
                j2 = j;
                if (TextUnit.m3943equalsimpl0(j2, fastMerge.getLineHeight())) {
                }
            } else {
                j2 = j;
            }
        ...
        long lineHeight = TextUnitKt.m3964isUnspecifiedR2X_6o(j) ? fastMerge.getLineHeight() : j2;
        if (textIndent2 == null) {
    ...
    public static final ParagraphStyle lerp(@NotNull ParagraphStyle start, @NotNull ParagraphStyle stop, float f) {
        ...
        long jM3275lerpTextUnitInheritableC3pnCVY = SpanStyleKt.m3275lerpTextUnitInheritableC3pnCVY(start.getLineHeight(), stop.getLineHeight(), f);
    ...
    public static final ParagraphStyle resolveParagraphStyleDefaults(@NotNull ParagraphStyle style, @NotNull LayoutDirection direction) {
        ...
        TextAlign textAlignM3664boximpl = TextAlign.m3664boximpl(style.getTextAlignOrDefault());
        TextDirection textDirectionM3677boximpl = TextDirection.m3677boximpl(TextStyleKt.m3342resolveTextDirectionYj3eThk(direction, style.getTextDirection()));
        long lineHeight = TextUnitKt.m3964isUnspecifiedR2X_6o(style.getLineHeight()) ? DefaultLineHeight : style.getLineHeight();
        ...
        LineBreak lineBreakM3592boximpl = LineBreak.m3592boximpl(style.getLineBreakOrDefault());
        Hyphens hyphensM3583boximpl = Hyphens.m3583boximpl(style.getHyphensOrDefault());
        TextMotion textMotion = style.getTextMotion();
        if (textMotion == null) {
            textMotion = TextMotion.INSTANCE.getStatic();
        }
        return new ParagraphStyle(textAlignM3664boximpl, textDirectionM3677boximpl, lineHeight, textIndent2, platformStyle, lineHeightStyle, lineBreakM3592boximpl, hyphensM3583boximpl, textMotion, (DefaultConstructorMarker) null);
    }
}
