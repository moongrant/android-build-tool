...
public final class XlogWriter implements LogWriter {
    ...
    public final void init(@NotNull WeakReference<Context> contextRef, @NotNull FileLogConfig fileLogConfig2, @NotNull CoroutineScope scope) {
        ...
        if (StringsKt.OooOO0O(logDir2, separator)) {
            strStringPlus = Intrinsics.stringPlus(logDir2, LOG_DIR_NAME);
        } else {
            strStringPlus = logDir2 + ((Object) separator) + LOG_DIR_NAME;
        }
        setLogDir(Intrinsics.stringPlus(strStringPlus, getEnvDirSuffix()));
        logCacheDir = Intrinsics.stringPlus(new File(strStringPlus).getAbsolutePath(), LOG_DIR_CACHE_SUFFIX);
        Intrinsics.checkNotNullExpressionValue(separator, "separator");
        if (StringsKt.OooOO0O(logDir2, separator)) {
            StringBuilder sbOooO0O0 = o000Oo0.OooO0O0(logDir2, CACHE_DIR_NAME);
            sbOooO0O0.append(envDirSuffix);
            string = sbOooO0O0.toString();
        } else {
            string = logDir2 + ((Object) separator) + CACHE_DIR_NAME + envDirSuffix;
        }
        setCacheDir(string);
        xlog = new Xlog();
        open();
            ...
            public final void invoke2() {
                ...
                LoggerKt.logD$default(null, new Function0<String>() { // from class: com.common.support.apm.xlog.XlogWriter$init$uploader$1.1
                    ...
                    public final String invoke() {
                        return Intrinsics.stringPlus("Before upload. Is the flush operation successful? ", Boolean.valueOf(zAppenderFlush));
                    }
                }, 1, null);
                if (zAppenderFlush) {
                    return;
        ...
        for (final byte[] bArr : convertor.convert((BaseModel) model)) {
            ...
            if (bArr.length > MAX_SIZE_OF_SINGLE_LINE_LOG) {
                LoggerKt.logD$default(null, new Function0<String>() { // from class: com.common.support.apm.xlog.XlogWriter$write$1$1
                    ...
                    @NotNull
                    public final String invoke() {
                        return "Log size is " + bArr.length + "B. The max size of single line log is 10 * 1024B, " + ((Object) model.getClass().getName());
                    }
                }, 1, null);
                INSTANCE.writeLog(MonitorErrorCodeKt.MONITOR_ERROR_XLOG_LINE_EXCEED_MAX_SIZE, "Xlog line size: " + bArr.length + "B, " + ((Object) model.getClass().getName()));
                z = false;
            } else {
                Xlog xlog3 = xlog;
                if (xlog3 == null) {
