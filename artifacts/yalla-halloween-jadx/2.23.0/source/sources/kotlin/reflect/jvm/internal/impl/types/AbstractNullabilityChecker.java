...
public final class AbstractNullabilityChecker {
        ...
        if (!((typeSystemContext.isClassType(type) && !typeSystemContext.isMarkedNullable(type)) || typeSystemContext.isDefinitelyNotNullType(type))) {
            ...
            while (!supertypesDeque.isEmpty()) {
                if (supertypesSet.size() > 1000) {
                    StringBuilder sbOooO00o = OooO00o.OooO00o("Too many supertypes for type: ", type, ". Supertypes = ");
                    sbOooO00o.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (supertypesPolicy2 == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker simpleTypeMarkerMo5506transformType = supertypesPolicy2.mo5506transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(simpleTypeMarkerMo5506transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo5506transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo5506transformType)) {
                                typeCheckerState.clear();
                            } else {
                                supertypesDeque.add(simpleTypeMarkerMo5506transformType);
                            }
                        }
                    }
                }
    ...
    public final boolean hasPathByNotMarkedNullableNodes(@NotNull TypeCheckerState state, @NotNull SimpleTypeMarker start, @NotNull TypeConstructorMarker end) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbOooO00o = OooO00o.OooO00o("Too many supertypes for type: ", start, ". Supertypes = ");
                sbOooO00o.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                throw new IllegalStateException(sbOooO00o.toString().toString());
            }
            SimpleTypeMarker current = supertypesDeque.pop();
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo5506transformType = supertypesPolicy.mo5506transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo5506transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo5506transformType);
