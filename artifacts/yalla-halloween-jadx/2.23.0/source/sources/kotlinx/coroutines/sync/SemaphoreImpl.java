...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM5779constructorimpl;
        boolean z;
        boolean z2;
        boolean z3;
        boolean z4;
        ...
        do {
            ...
            while (true) {
                if (segment.getId() >= j && !segment.getRemoved()) {
                    objM5779constructorimpl = SegmentOrClosed.m5779constructorimpl(segment);
                    break;
                }
                Object obj = segment.get_next();
                if (obj == ConcurrentLinkedListKt.CLOSED) {
                    objM5779constructorimpl = SegmentOrClosed.m5779constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            if (!SegmentOrClosed.m5784isClosedimpl(objM5779constructorimpl)) {
                Segment segmentM5782getSegmentimpl = SegmentOrClosed.m5782getSegmentimpl(objM5779constructorimpl);
                while (true) {
                    Segment segment2 = (Segment) this.tail;
                    if (segment2.getId() < segmentM5782getSegmentimpl.getId()) {
                        if (!segmentM5782getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                            z3 = false;
                            break;
                        }
                        AtomicReferenceFieldUpdater atomicReferenceFieldUpdater = tail$FU;
                        while (true) {
                            if (atomicReferenceFieldUpdater.compareAndSet(this, segment2, segmentM5782getSegmentimpl)) {
                                z4 = true;
                                break;
                            }
                        ...
                        } else if (segmentM5782getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                            segmentM5782getSegmentimpl.remove();
                        }
                    }
                    z3 = true;
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m5782getSegmentimpl(objM5779constructorimpl);
