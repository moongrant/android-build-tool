...
public class ComposeTypeResolutionInterceptorExtension implements TypeResolutionInterceptorExtension {
    ...
    public AnonymousFunctionDescriptor interceptFunctionLiteralDescriptor(@NotNull KtLambdaExpression expression, @NotNull ExpressionTypingContext context, @NotNull AnonymousFunctionDescriptor descriptor) {
        if (descriptor.isSuspend() || AnnotationUtilsKt.hasComposableAnnotation((Annotated) descriptor) || !ComposableCallCheckerKt.hasComposableExpectedType((ResolutionContext) context, (KtExpression) expression)) {
            return descriptor;
        }
        AnonymousFunctionDescriptor anonymousFunctionDescriptorAnnotateAsComposable = AnnotationUtilsKt.annotateAsComposable(descriptor, DescriptorUtilsKt.getModule(context.scope.getOwnerDescriptor()));
        context.trace.record(FrontendWritableSlices.INSTANCE.getINFERRED_COMPOSABLE_DESCRIPTOR(), anonymousFunctionDescriptorAnnotateAsComposable, Boolean.TRUE);
    ...
    public KotlinType interceptType(@NotNull KtElement element, @NotNull ExpressionTypingContext context, @NotNull KotlinType resultType) {
        ...
        if (!ComposableCallCheckerKt.hasComposableAnnotation(KtPsiUtilKt.getAnnotationEntries(ktExpression), context.trace.getBindingContext()) && !ComposableCallCheckerKt.hasComposableExpectedType((ResolutionContext) context, ktExpression)) {
            return resultType;
        }
        context.trace.record(FrontendWritableSlices.INSTANCE.getINFERRED_COMPOSABLE_LITERAL(), element, Boolean.TRUE);
        return AnnotationUtilsKt.makeComposable(resultType, DescriptorUtilsKt.getModule(context.scope.getOwnerDescriptor()));
    }
}
