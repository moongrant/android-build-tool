...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    iMax = Math.max(iMax, isVertical ? IntSize.m3934getHeightimpl(visibleItemsInfo.get(i).getSize()) : IntSize.m3935getWidthimpl(visibleItemsInfo.get(i).getSize()));
                    i++;
                }
                i2 += iMax;
                i3++;
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (lazyGridItemInfo2 != null) {
            return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m3894getYimpl(lazyGridItemInfo2.getOffset()) : IntOffset.m3893getXimpl(lazyGridItemInfo2.getOffset()));
        }
        return null;
    }
