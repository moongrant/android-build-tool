...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m616childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        return this.isVertical ? Constraints.INSTANCE.m3753fixedWidthOenEA2s(iCoerceAtLeast) : Constraints.INSTANCE.m3752fixedHeightOenEA2s(iCoerceAtLeast);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            int iM595getCurrentLineSpanimpl = GridItemSpan.m595getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i3).getPackedValue());
            LazyGridMeasuredItem lazyGridMeasuredItemM615getAndMeasure3p2s80s = this.measuredItemProvider.m615getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i3, i, m616childConstraintsJhjzzOo$foundation_release(i2, iM595getCurrentLineSpanimpl));
            i2 += iM595getCurrentLineSpanimpl;
            Unit unit = Unit.INSTANCE;
            lazyGridMeasuredItemArr[i3] = lazyGridMeasuredItemM615getAndMeasure3p2s80s;
        }
    ...
    public final long m617itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m616childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
}
