...
public final class TextDelegate {
    ...
    public static /* synthetic */ TextLayoutResult m791layoutNN6EwU$default(TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
        if ((i & 4) != 0) {
            textLayoutResult = null;
        }
        return textDelegate.m794layoutNN6EwU(j, layoutDirection, textLayoutResult);
    }

    /* JADX INFO: renamed from: layoutText-K40F9xA, reason: not valid java name */
    private final MultiParagraph m792layoutTextK40F9xA(long constraints, LayoutDirection layoutDirection) {
        layoutIntrinsics(layoutDirection);
        int iM3745getMinWidthimpl = Constraints.m3745getMinWidthimpl(constraints);
        boolean z = false;
        int iM3743getMaxWidthimpl = ((this.softWrap || TextOverflow.m3721equalsimpl0(this.overflow, TextOverflow.INSTANCE.m3729getEllipsisgIe3tQ8())) && Constraints.m3739getHasBoundedWidthimpl(constraints)) ? Constraints.m3743getMaxWidthimpl(constraints) : Integer.MAX_VALUE;
        if (!this.softWrap && TextOverflow.m3721equalsimpl0(this.overflow, TextOverflow.INSTANCE.m3729getEllipsisgIe3tQ8())) {
            z = true;
        }
        int i = z ? 1 : this.maxLines;
        if (iM3745getMinWidthimpl != iM3743getMaxWidthimpl) {
            iM3743getMaxWidthimpl = RangesKt.coerceIn(getMaxIntrinsicWidth(), iM3745getMinWidthimpl, iM3743getMaxWidthimpl);
        }
        return new MultiParagraph(getNonNullIntrinsics(), ConstraintsKt.Constraints$default(0, iM3743getMaxWidthimpl, 0, Constraints.m3742getMaxHeightimpl(constraints), 5, null), i, TextOverflow.m3721equalsimpl0(this.overflow, TextOverflow.INSTANCE.m3729getEllipsisgIe3tQ8()), null);
    }
    ...
    public final TextLayoutResult m794layoutNN6EwU(long constraints, @NotNull LayoutDirection layoutDirection, @Nullable TextLayoutResult prevResult) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        if (prevResult != null && TextLayoutHelperKt.m827canReuse7_7YC6M(prevResult, this.text, this.style, this.placeholders, this.maxLines, this.softWrap, this.overflow, this.density, layoutDirection, this.fontFamilyResolver, constraints)) {
            return prevResult.m3291copyO0kMr_c(new TextLayoutInput(prevResult.getLayoutInput().getText(), this.style, prevResult.getLayoutInput().getPlaceholders(), prevResult.getLayoutInput().getMaxLines(), prevResult.getLayoutInput().getSoftWrap(), prevResult.getLayoutInput().getOverflow(), prevResult.getLayoutInput().getDensity(), prevResult.getLayoutInput().getLayoutDirection(), prevResult.getLayoutInput().getFontFamilyResolver(), constraints, (DefaultConstructorMarker) null), ConstraintsKt.m3754constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getWidth()), TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getHeight()))));
        }
        MultiParagraph multiParagraphM792layoutTextK40F9xA = m792layoutTextK40F9xA(constraints, layoutDirection);
        return new TextLayoutResult(new TextLayoutInput(this.text, this.style, this.placeholders, this.maxLines, this.softWrap, this.overflow, this.density, layoutDirection, this.fontFamilyResolver, constraints, (DefaultConstructorMarker) null), multiParagraphM792layoutTextK40F9xA, ConstraintsKt.m3754constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(multiParagraphM792layoutTextK40F9xA.getWidth()), TextDelegateKt.ceilToIntPx(multiParagraphM792layoutTextK40F9xA.getHeight()))), null);
    }
    ...
    public /* synthetic */ TextDelegate(AnnotatedString annotatedString, TextStyle textStyle, int i, int i2, boolean z, int i3, Density density, FontFamily.Resolver resolver, List list, int i4, DefaultConstructorMarker defaultConstructorMarker) {
        this(annotatedString, textStyle, (i4 & 4) != 0 ? Integer.MAX_VALUE : i, (i4 & 8) != 0 ? 1 : i2, (i4 & 16) != 0 ? true : z, (i4 & 32) != 0 ? TextOverflow.INSTANCE.m3728getClipgIe3tQ8() : i3, density, resolver, (i4 & PSKKeyManager.MAX_KEY_LENGTH_BYTES) != 0 ? CollectionsKt.emptyList() : list, null);
    }
}
