...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (!textLayoutResult.getDidOverflowHeight() || textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                ...
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m3934getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m3934getHeightimpl(textLayoutResult.getSize())) {
                    lineCount--;
                }
            }
    ...
    public long mo883getHandlePositiondBAh8RU(@NotNull Selection selection, boolean isStartHandle) {
    ...
    public long mo884getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult textLayoutResultInvoke = this.layoutResultCallback.invoke();
        if (textLayoutResultInvoke == null) {
    ...
    public Selection getSelectAllSelection() {
        ...
        return MultiWidgetSelectionDelegateKt.m887getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, textLayoutResultInvoke.getLayoutInput().getText().length()), false, getSelectableId(), textLayoutResultInvoke);
    }
    ...
    public Pair<Selection, Boolean> mo885updateSelectionqCDeeow(long startHandlePosition, long endHandlePosition, @Nullable Offset previousHandlePosition, boolean isStartHandle, @NotNull LayoutCoordinates containerLayoutCoordinates, @NotNull SelectionAdjustment adjustment, @Nullable Selection previousSelection) {
        ...
        long jMo2812localPositionOfR5De75A = containerLayoutCoordinates.mo2812localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m1463getZeroF1C5BW0());
        return MultiWidgetSelectionDelegateKt.m889getTextSelectionInfoyM0VcXU(textLayoutResultInvoke, Offset.m1451minusMKHz9U(startHandlePosition, jMo2812localPositionOfR5De75A), Offset.m1451minusMKHz9U(endHandlePosition, jMo2812localPositionOfR5De75A), previousHandlePosition != null ? Offset.m1436boximpl(Offset.m1451minusMKHz9U(previousHandlePosition.getPackedValue(), jMo2812localPositionOfR5De75A)) : null, getSelectableId(), adjustment, previousSelection, isStartHandle);
    }
