...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo1337alignKFBX0sM(long size, long space, @NotNull LayoutDirection layoutDirection) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        long jIntSize = IntSizeKt.IntSize(IntSize.m3935getWidthimpl(space) - IntSize.m3935getWidthimpl(size), IntSize.m3934getHeightimpl(space) - IntSize.m3934getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(MathKt.roundToInt((this.horizontalBias + f) * (IntSize.m3935getWidthimpl(jIntSize) / 2.0f)), MathKt.roundToInt((f + this.verticalBias) * (IntSize.m3934getHeightimpl(jIntSize) / 2.0f)));
    }
