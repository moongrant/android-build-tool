...
public final class BitmapPainter extends Painter {
    ...
    private final long m2209validateSizeN5eqBDc(long srcOffset, long srcSize) {
        if (IntOffset.m3893getXimpl(srcOffset) >= 0 && IntOffset.m3894getYimpl(srcOffset) >= 0 && IntSize.m3935getWidthimpl(srcSize) >= 0 && IntSize.m3934getHeightimpl(srcSize) >= 0 && IntSize.m3935getWidthimpl(srcSize) <= this.image.getWidth() && IntSize.m3934getHeightimpl(srcSize) <= this.image.getHeight()) {
            return srcSize;
        }
    ...
    public boolean equals(@Nullable Object other) {
        ...
        return Intrinsics.areEqual(this.image, bitmapPainter.image) && IntOffset.m3892equalsimpl0(this.srcOffset, bitmapPainter.srcOffset) && IntSize.m3933equalsimpl0(this.srcSize, bitmapPainter.srcSize) && FilterQuality.m1771equalsimpl0(this.filterQuality, bitmapPainter.filterQuality);
    }
    ...
    public long mo2211getIntrinsicSizeNHjbRc() {
        return IntSizeKt.m3945toSizeozmzZPI(this.size);
    }

    public int hashCode() {
        return FilterQuality.m1772hashCodeimpl(this.filterQuality) + ((IntSize.m3936hashCodeimpl(this.srcSize) + ((IntOffset.m3895hashCodeimpl(this.srcOffset) + (this.image.hashCode() * 31)) * 31)) * 31);
    }

    @Override // androidx.compose.ui.graphics.painter.Painter
    public void onDraw(@NotNull DrawScope drawScope) {
        Intrinsics.checkNotNullParameter(drawScope, "<this>");
        OooO0O0.OooOoO(drawScope, this.image, this.srcOffset, this.srcSize, 0L, IntSizeKt.IntSize(MathKt.roundToInt(Size.m1516getWidthimpl(drawScope.mo2118getSizeNHjbRc())), MathKt.roundToInt(Size.m1513getHeightimpl(drawScope.mo2118getSizeNHjbRc()))), this.alpha, null, this.colorFilter, 0, this.filterQuality, 328, null);
    }

    /* JADX INFO: renamed from: setFilterQuality-vDHp3xo$ui_graphics_release, reason: not valid java name */
    public final void m2212setFilterQualityvDHp3xo$ui_graphics_release(int i) {
        this.filterQuality = i;
    }

    @NotNull
    public String toString() {
        return "BitmapPainter(image=" + this.image + ", srcOffset=" + ((Object) IntOffset.m3900toStringimpl(this.srcOffset)) + ", srcSize=" + ((Object) IntSize.m3938toStringimpl(this.srcSize)) + ", filterQuality=" + ((Object) FilterQuality.m1773toStringimpl(this.filterQuality)) + ')';
    }
    ...
    public /* synthetic */ BitmapPainter(ImageBitmap imageBitmap, long j, long j2, int i, DefaultConstructorMarker defaultConstructorMarker) {
        this(imageBitmap, (i & 2) != 0 ? IntOffset.INSTANCE.m3903getZeronOccac() : j, (i & 4) != 0 ? IntSizeKt.IntSize(imageBitmap.getWidth(), imageBitmap.getHeight()) : j2, null);
    }
    ...
    private BitmapPainter(ImageBitmap image, long j, long j2) {
        ...
        this.srcOffset = j;
        this.srcSize = j2;
        this.filterQuality = FilterQuality.INSTANCE.m1776getLowfv9h1I();
        this.size = m2209validateSizeN5eqBDc(j, j2);
        this.alpha = 1.0f;
    }
}
