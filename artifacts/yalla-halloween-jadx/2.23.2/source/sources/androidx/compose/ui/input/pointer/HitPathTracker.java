...
public final class HitPathTracker {
    ...
    public final void m2628addHitPathKNwqfcY(long pointerId, @NotNull List<? extends Modifier.Node> pointerInputNodes) {
        Node node;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (z) {
                ...
                if (node3 != null) {
                    node3.markIsIn();
                    if (!node3.getPointerIds().contains(PointerId.m2679boximpl(pointerId))) {
                        node3.getPointerIds().add(PointerId.m2679boximpl(pointerId));
                    }
                    nodeParent = node3;
                } else {
                    z = false;
                    Node node4 = new Node(node2);
                    node4.getPointerIds().add(PointerId.m2679boximpl(pointerId));
                    nodeParent.getChildren().add(node4);
                    nodeParent = node4;
                }
            } else {
                Node node5 = new Node(node2);
                node5.getPointerIds().add(PointerId.m2679boximpl(pointerId));
