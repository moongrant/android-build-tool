...
public final class VelocityTrackerKt {
    ...
    public static final void addPointerInputChange(@NotNull VelocityTracker velocityTracker, @NotNull PointerInputChange event) {
        ...
        if (PointerEventKt.changedToDownIgnoreConsumed(event)) {
            velocityTracker.m2780setCurrentPointerPositionAccumulatork4lQ0M$ui_release(event.getPosition());
        ...
        while (i < size) {
            ...
            long jM1451minusMKHz9U = Offset.m1451minusMKHz9U(historicalChange.getPosition(), previousPosition);
            long position = historicalChange.getPosition();
            velocityTracker.m2780setCurrentPointerPositionAccumulatork4lQ0M$ui_release(Offset.m1452plusMKHz9U(velocityTracker.getCurrentPointerPositionAccumulator(), jM1451minusMKHz9U));
            velocityTracker.m2777addPositionUv8p0NA(historicalChange.getUptimeMillis(), velocityTracker.getCurrentPointerPositionAccumulator());
            i++;
            previousPosition = position;
        }
        velocityTracker.m2780setCurrentPointerPositionAccumulatork4lQ0M$ui_release(Offset.m1452plusMKHz9U(velocityTracker.getCurrentPointerPositionAccumulator(), Offset.m1451minusMKHz9U(event.getPosition(), previousPosition)));
        velocityTracker.m2777addPositionUv8p0NA(event.getUptimeMillis(), velocityTracker.getCurrentPointerPositionAccumulator());
    }
    ...
    public static final float calculateImpulseVelocity(float[] fArr, float[] fArr2, int i, boolean z) {
        ...
        for (int i3 = i2; i3 > 0; i3--) {
            ...
            if (!(fArr2[i3] == fArr2[i4])) {
                float fSignum = Math.signum(f) * ((float) Math.sqrt(Math.abs(f) * 2));
                float f4 = (z ? -fArr[i4] : fArr[i3] - fArr[i4]) / (fArr2[i3] - fArr2[i4]);
                float fAbs = (Math.abs(f4) * (f4 - fSignum)) + f;
                if (i3 == i2) {
                    fAbs *= 0.5f;
                }
                f = fAbs;
