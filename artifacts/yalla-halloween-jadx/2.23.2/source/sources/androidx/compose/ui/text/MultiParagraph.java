...
import androidx.compose.ui.graphics.o00Oo00;
...
public final class MultiParagraph {
    ...
    public static /* synthetic */ void m3209paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m1717getUnspecified0d7_KjU();
        }
        multiParagraph.m3214paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    private final void requireIndexInRange(int offset) {
        ...
        sbOooO0O0.append(getAnnotatedString().length());
        sbOooO0O0.append(')');
    ...
    private final void requireLineIndexInRange(int lineIndex) {
        ...
        if (!z) {
            throw new IllegalArgumentException(androidx.compose.foundation.layout.OooO0O0.OooO00o(o0000O0.OooO0O0("lineIndex(", lineIndex, ") is out of bounds [0, "), this.lineCount, ')').toString());
        }
    }
    ...
    public final int m3211getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m1448getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m1448getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m1448getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        return paragraphInfo.getLength() == 0 ? Math.max(0, paragraphInfo.getStartIndex() - 1) : paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo3183getOffsetForPositionk4lQ0M(paragraphInfo.m3217toLocalMKHz9U(position)));
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int iFindParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); iFindParagraphByIndex < size; iFindParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                o00Oo00.OooO0oO(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final long m3212getWordBoundaryjx7JFs(int offset) {
        requireIndexInRangeInclusiveEnd(offset);
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(offset == getAnnotatedString().length() ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, offset));
        return paragraphInfo.m3216toGlobalGEjPoXI(paragraphInfo.getParagraph().mo3184getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
    ...
    public final void m3213paintLG529CI(@NotNull Canvas canvas, long color, @Nullable Shadow shadow, @Nullable TextDecoration decoration, @Nullable DrawStyle drawStyle, int blendMode) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo3185paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
    ...
    public final /* synthetic */ void m3214paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo3186paintRPmYEkk(canvas, color, shadow, decoration);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m3215painthn5TExg(@NotNull Canvas canvas, @NotNull Brush brush, float alpha, @Nullable Shadow shadow, @Nullable TextDecoration decoration, @Nullable DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m3542drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        int i2 = 0;
        if (Constraints.m3745getMinWidthimpl(j) == 0 && Constraints.m3744getMinHeightimpl(j) == 0) {
            ...
            while (true) {
                ...
                if (Constraints.m3738getHasBoundedHeightimpl(j)) {
                    iM3742getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m3742getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
                } else {
                    iM3742getMaxHeightimpl = Constraints.m3742getMaxHeightimpl(j);
                }
                Paragraph paragraphM3220Paragraph_EkL_Y = ParagraphKt.m3220Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, iM3743getMaxWidthimpl, 0, iM3742getMaxHeightimpl, 5, null), this.maxLines - i3, z);
                float height = paragraphM3220Paragraph_EkL_Y.getHeight() + f;
                int lineCount = paragraphM3220Paragraph_EkL_Y.getLineCount() + i3;
                arrayList.add(new ParagraphInfo(paragraphM3220Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
                if (paragraphM3220Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                    i3 = lineCount;
                    f = height;
                    z2 = true;
                    break;
            ...
            this.width = Constraints.m3743getMaxWidthimpl(j);
