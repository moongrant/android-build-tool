...
public final class AbstractNullabilityChecker {
        ...
        if (!((typeSystemContext.isClassType(type) && !typeSystemContext.isMarkedNullable(type)) || typeSystemContext.isDefinitelyNotNullType(type))) {
            ...
            while (!supertypesDeque.isEmpty()) {
                if (supertypesSet.size() > 1000) {
                    StringBuilder sbOooO0OO = OooO00o.OooO0OO("Too many supertypes for type: ", type, ". Supertypes = ");
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (supertypesPolicy2 == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker simpleTypeMarkerMo5496transformType = supertypesPolicy2.mo5496transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(simpleTypeMarkerMo5496transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo5496transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo5496transformType)) {
    ...
    public final boolean hasPathByNotMarkedNullableNodes(@NotNull TypeCheckerState state, @NotNull SimpleTypeMarker start, @NotNull TypeConstructorMarker end) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbOooO0OO = OooO00o.OooO0OO("Too many supertypes for type: ", start, ". Supertypes = ");
                sbOooO0OO.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                throw new IllegalStateException(sbOooO0OO.toString().toString());
            }
            SimpleTypeMarker current = supertypesDeque.pop();
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo5496transformType = supertypesPolicy.mo5496transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo5496transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo5496transformType);
