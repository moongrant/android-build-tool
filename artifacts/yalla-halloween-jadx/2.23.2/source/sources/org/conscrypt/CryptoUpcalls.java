...
final class CryptoUpcalls {
    ...
    private static byte[] rsaOpWithPrivateKey(PrivateKey privateKey, int i, int i2, byte[] bArr) {
        ...
        if (cipher == null) {
            Iterator<Provider> it = getExternalProviders(o00O00OO.OooO00o("Cipher.", strConcat)).iterator();
            while (it.hasNext()) {
                try {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Exception while decrypting message with " + privateKey.getAlgorithm() + " private key using " + strConcat + CertificateUtil.DELIMITER, (Throwable) e2);
            return null;
        }
    }
    ...
    private static byte[] signDigestWithPrivateKey(PrivateKey privateKey, byte[] bArr, String str) {
        ...
        if (signature == null) {
            Iterator<Provider> it = getExternalProviders(o00O00OO.OooO00o("Signature.", str)).iterator();
