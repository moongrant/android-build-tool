(function installYallaOfficialSpaArchive() {
  "use strict";
  var SLUG = "yalla-happy-ferris-wheel-castle-2025";
  var API_STATUS = 400;
  var ledger = [];
  var errors = [];
  ...
  function body(path, status) {
    ...
    return {code:status,data:null,list:[],res:[],count:0,message:"Archived official SPA: historical API data is unavailable"};
  }
  ...
  if (nativeFetch) window.fetch = function(input, init) {
    ...
    var path = endpoint(url); var status = isAnalytics(path) ? 200 : (isWrite(method,path) ? 410 : API_STATUS);
