...
if marker not in src:
    raise SystemExit('fixture setup marker not found')
src = src.replace(marker, marker + fixtures, 1)
...
prefetch = r'''async function prefetchWave3Resources(){
  ...
  async function fetchFirst(candidates,accept){
  ...
  for(const url of targets){
    ...
    const got=await fetchFirst(variants(url),accept);
...
if old not in src:
    raise SystemExit('prefetch call anchor not found')
src = src.replace(old, new, 1)

old = "staticClosureAdded:closureAdded,activityConfigAdded,captureErrors:captureErrors.length"
new = "staticClosureAdded:closureAdded,activityConfigAdded,wave3Resources,captureErrors:captureErrors.length"
if old not in src:
    raise SystemExit('manifest field anchor not found')
src = src.replace(old, new, 1)

p.write_text(src, encoding='utf-8')
print('patched wave3 archiver v2')
